...
# creating an account 	
	def new 
		...
		if !session[:user].nil?  # redirect user to their photos if he or she visits the registration screen but is already logged in
			#redirect_to(:controller => :flash_teams, :action => :index, :user_id => session[:user].id)
	...
	def login 
		...
		# redirect user to their photos if he or she visits the login screen but is already logged in
		if !session[:user].nil?
			redirect_to(:controller => :flash_teams, :action => :index)
		end		
