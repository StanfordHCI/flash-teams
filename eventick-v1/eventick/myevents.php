...
$result = mysql_query("SELECT distinct t1.*,t2.IsFavorite,t2.IsAttending FROM tbl_events t1 LEFT JOIN tbl_eventresponse t2 on t1.Event_ID= t2.Event_ID WHERE Created_By='".$_SESSION['userid']."'")
or die(mysql_error());
}
$cnt=0;
...
echo "<p style='text-align:right;'><strong>From ".$row['Event_From']." to ".$row['Event_To']."</strong></p>";
...
if($cnt==0)
	echo '<div class="alert alert-error"> <button type="button" class="close" data-dismiss="alert">&times;</button> <strong>! You have not added any events yet</strong></div>';
