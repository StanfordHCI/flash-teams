...
function fillPopover(newmouseX, numHours) {
    //Find the start time
...
function addEventToJSON(x, y, groupNum, showPopover)
{
    //Find the start time
    var startHr = (x-(x%100))/100;
    var startMin = (x%100)/25*15;
>>>>>>> jay-timeline-extend
    if(startMin == 57.599999999999994) {
        startHr++;
        startMin = 0;
...
=======
    ...
    addEventPopover(startHr, startMin, groupNum, showPopover);
    overlayOn();
...
<<<<<<< HEAD
    addEventPopover(startHr, startMin, numHours);
    overlayOn();
}

//Draws event and adds it to the JSON when the timeline is clicked and overlay is off
function mousedown(numHours) {
	if (numHours == null) {
		numHours = 1;
	}
...
//Creates graphical element from array of data (task_rectangles)
function  drawEvents(x, y, d) {
    var task_g;
    var groupNum;
    if (d === null) {
...
<<<<<<< HEAD
function addEventPopover(startHr, startMin, numHours) {
=======
function addEventPopover(startHr, startMin, groupNum, showPopover) {
>>>>>>> jay-timeline-extend
    //Add Popovers
    timeline_svg.selectAll("#rect_" + groupNum).each(
        function(d) {
            $(this).popover({
                ...
                id: '"popover' + groupNum + '"',
                trigger: "click",
                title: '<input type ="text" name="eventName" id="eventName_' + groupNum + '" placeholder="New Event" >',
...
=======
                +'Hours: <input type = "number" id="hours_' + groupNum + '" placeholder="1" min="0" style="width:35px"/>          ' 
                +'Minutes: <input type = "number" id = "minutes_' + groupNum + '" placeholder="00" style="width:35px" min="0" step="15" max="45"/><br>'
                +'<br>Members<br><input class="eventMemberInput" id="eventMember_' + groupNum + '" style="width:140px" type="text" name="members" onclick="addMemAuto()">'
                +'<button class="btn" type="button" id="addEventMember_' + groupNum + '" onclick="addEventMember(' + event_counter +')">+Add</button>'
                +'<ul class="nav nav-pills" id="eventMembers_' + groupNum + '"> </ul>'
                +'Notes: <textarea rows="3" id="notes_' + groupNum + '"></textarea>'
